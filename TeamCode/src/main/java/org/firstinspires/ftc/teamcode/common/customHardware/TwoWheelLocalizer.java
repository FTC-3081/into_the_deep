...
// Ripped from gobilda git and renamed so reading the code makes sense
// Also interfaced with my geometry package for consistency
// Also now y is forward and x is sideways and heading is theta because i'm not a flipping nutjob
...
public class TwoWheelLocalizer extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {

    private int deviceStatus   = 0;
    private int loopTime       = 0;
    private int xEncoderValue  = 0;
    private int yEncoderValue  = 0;
    private float xState = 0;
    private float yState = 0;
    private float tState = 0;
    private float xVelocity    = 0;
    private float yVelocity    = 0;
    private float tVelocity = 0;

    //i2c address of the device
    public static final byte DEFAULT_ADDRESS = 0x31;

    public TwoWheelLocalizer(I2cDeviceSynchSimple deviceClient, boolean deviceClientIsOwned) {
...
//    }
    ...
    public void read(){
        ...
        xState = byteArrayToFloat(Arrays.copyOfRange(bArr, 16,20), ByteOrder.LITTLE_ENDIAN);
        yState = byteArrayToFloat(Arrays.copyOfRange(bArr, 20,24), ByteOrder.LITTLE_ENDIAN);
        tState = byteArrayToFloat(Arrays.copyOfRange(bArr, 24,28), ByteOrder.LITTLE_ENDIAN);
        ...
        tVelocity = byteArrayToFloat(Arrays.copyOfRange(bArr, 36,40), ByteOrder.LITTLE_ENDIAN);
    }
...
//    }
     ...
     */
    public void resetIMU(){writeInt(Register.DEVICE_CONTROL,1<<0);}
    ...
    public void setEncoderDirections(EncoderDirection xEncoder, EncoderDirection yEncoder){
        ...
        if (xEncoder == EncoderDirection.REVERSE) {
        ...
        if (yEncoder == EncoderDirection.REVERSE){
     ...
     */
    private void setYawScalar(double yawOffset){
     ...
     */
    private Pose2D setPosition(Pose2D pos){
     ...
     */
    private int getDeviceID(){return readInt(Register.DEVICE_ID);}

    /**
     * @return the firmware version of the Odometry Computer
     */
    private int getDeviceVersion(){return readInt(Register.DEVICE_VERSION); }

    private float getYawScalar(){return readFloat(Register.YAW_SCALAR); }
     ...
     */
    private float getXOffset(){return readFloat(Register.X_POD_OFFSET);}

    /**
     * <strong> This uses its own I2C read, avoid calling this every loop. </strong>
     * @return the user-set offset for the Y (strafe) pod
     */
    private float getYOffset(){return readFloat(Register.Y_POD_OFFSET);}
    ...
    public String getTelemetry(){
        return "x: " +  (int) xState + ", y: " + (int) yState + ", t: " + (int) toDegrees(tState);
    }
