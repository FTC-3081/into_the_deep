...
public class ExtensionFF extends DirectionalFF{
    ...
    private final double KfMin, KfMax, Kg;
     ...
     * @param gravity the necessary motor power to start raising the arm at 0
     ...
     */
    public ExtensionFF(double frictionMin, double frictionMax, double gravity, DoubleSupplier position, DoubleSupplier scalar, double KfDead, Range deadRange){
        super(frictionMin, KfDead, deadRange);
        this.position = position;
        this.scalar = scalar;
        Kg = gravity - frictionMin;
        KfMin = frictionMin;
        KfMax = frictionMax - frictionMin;
    }

    public double get(double error){
        set(KfMin + KfMax * scalar.getAsDouble() * sin(position.getAsDouble()));
